...
usage() {
    if [[ "$lang" == "fr" ]]; then
        ...
        echo "  -o <fichier>      Spécifie le fichier de sortie. Par défaut : install.txt."
        echo
        ...
        echo "  -o <file>         Specify the output file. Default: install.txt."
        echo
    ...
    if [[ "$line" == Start-Date:* ]]; then
        ...
        for package in $packages; do
            ...
            if [[ -n "$version" ]]; then
                ...
                echo -e "$formatted_date\t$package\t$version" >> "$output_file"
            fi
        done
    fi
...
if [[ "$lang" == "fr" ]]; then
    echo -e "\nTraitement terminé.\n"
    echo -e "Total de packages trouvés: $n\n"
else
    echo -e "\nProcessing complete.\n"
    echo -e "Total packages found: $n\n"
fi
